...
# service restarts, firmware writes. Records bounded scheduling metadata only, without image buffers.
set -u
...
{
  echo 'SCAMERA ZSL live trace v1'; date -u; echo "PID=$target"
...
helper=$!
echo 'ZSL v1: собираются только параметры очереди и брекета, без изображений.'
...
while [ "$(now)" -lt "$deadline" ]; do
  if grep -q '"event":"ready"' "$run/trace.log" && [ "$announced" = 0 ]; then
    ...
    echo 'Затем вернись в Termux. Сборщик остановится примерно через 10 секунд.'
    announced=1
  fi
