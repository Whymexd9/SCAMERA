...
(() => {
  ...
  function emit(event, fields={}) {
    ...
    if (emitted+line.length>4*1024*1024) { stop('byte_limit'); return; }
  ...
  function install(m) {
    ...
    hooks.push(Interceptor.attach(entry,{
      onEnter(args) {
        ...
        const selected=++seen%30===1&&samples<16;
        ...
        try {
          ...
          emit('enter',{sample:s.id,thread:this.threadId,input:bytes(input,0xe8),
      ...
      onLeave(ret) {
        ...
        if(samples>=16)stop('sample_limit');
      }
    }));
    emit('ready');
  }
  ...
  setTimeout(()=>stop('30_second_limit'),30000);
})();
