...
rule='allow hal_camera_default tmpfs file { read write open getattr map execute }'
backend=''; policy_tool=''
...
echo "RULE=$rule"
echo 'Scope: all tmpfs:file objects accessed by hal_camera_default, not one filename.'
