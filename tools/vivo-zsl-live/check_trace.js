...
function fixture(wrong=false, partial=false, attachFails=false, missingDelivery=false) {
  ...
  return{memory,events,timers,invoke,P,addQueue(){observer.onAdded({name:'libvcf_session.so',path:'/vendor/lib64/libvcf_session.so',base:new P(0x20000000),findExportByName(symbol){return new P(symbol.includes('getPastAndNextBuffers')?0x2012bb14:0x2012b6f8);}});}};
}
...
assert.deepEqual(f.events.filter(e=>e.event==='hook_installed').map(e=>e.address).sort(),['0x10101f48','0x2012b6f8','0x2012bb14']);
...
console.log('PASS: missing delivery export refused before full readiness; unknown queues ignored.');
