 ...
 *out=calloc(1,0x5dd0);
 if(*out && strcmp(getenv("TEST_FAULT"),"ymode")!=0)static_cast<unsigned char*>(*out)[0x2b0]=2;
 if(*out && strcmp(getenv("TEST_FAULT"),"config")==0)static_cast<unsigned char*>(*out)[0x2b8]=1;
 return *out?0:6;
}
...
''')
    ...
    profile=f'<swispConfig><yModelParameters>{y}</yModelParameters><auxYModelParameters>{y}</auxYModelParameters><uvModelParameters>{uv}</uvModelParameters><postSharpeningParameters>{sharp}</postSharpeningParameters></swispConfig>'
    (config/'softpqe_configs.xml').write_text('<softpqeConfig><processMode>0</processMode><enableSuperResolution>2</enableSuperResolution><enableUsm>1</enableUsm><enableSharpen>0</enableSharpen><enablePartialSharpen>0</enablePartialSharpen></softpqeConfig>')
    for suffix in ['', '_portrait', '_skin']:(config/f'softpqe_configs_master_2x{suffix}.xml').write_text(profile)
    for fault in ['none','platform','bypass','error','overrun','descriptor','stride','config','ymode']:
        case=d/fault;case.mkdir();out=case/'output';out.write_bytes(b'')
        p=subprocess.run([str(worker),'--softpqe',str(lib),str(config),str(inp),str(out),'32','32','64','64','17','100','2'],env={**os.environ,'TEST_FAULT':fault},capture_output=True,text=True,timeout=10)
        if fault=='none':
    ...
    # templates stay unchanged, and native Y is preserved while chroma comes only from the source.
    before={p.name:p.read_bytes() for p in config.iterdir()}
    inp.write_bytes(bytes([64])*(32*32*3//2));out=job/'output';out.write_bytes(b'')
    p=subprocess.run([str(worker),'--softpqe',str(lib),str(config),str(inp),str(out),'32','32','64','64','17','100','2'],
        env={**os.environ,'TEST_FAULT':'none','TEST_PROFILE':str(job)},capture_output=True,text=True,timeout=10)
    assert p.returncode==0,(p.stdout,p.stderr)
    assert out.read_bytes()==bytes([128])*(64*64)+bytes([64])*(64*64//2)
    assert before=={p.name:p.read_bytes() for p in config.iterdir()}
    assert ET.parse(job/'softpqe_configs.xml').findtext('enableUsm')=='0'
    assert ET.parse(job/'softpqe_configs_master_2x.xml').findtext('yModelParameters/noise1Level')=='0'
    assert ET.parse(job/'softpqe_configs.xml').findtext('processMode')=='2'
    assert 'SR-only v3 output' in p.stdout
    print('PASS worker tuning: private configs -> native Init, SR-only native Y and source chroma, unchanged firmware templates')
