...
struct ProfileReference {
    ...
    void execute(){
        ...
        auto transfer=NormalVst(sensitivity);VstLuts luts;
        ...
        for(size_t f=0;f<Frames;++f){
            ...
            for(int y=0;y<288;++y)for(int x=0;x<288;++x){
                ...
                v+=noise.normal()*std::sqrt(transfer.shot*v+transfer.variance);
        ...
        for(int y=0;y<576;++y)for(int x=0;x<576;++x)for(int c=0;c<3;++c){
            int xx=(red&1)?575-x:x,yy=(red&2)?575-y:y;
            int channel=fault==1?2-c:c;
            float v=chartValue(chart,channel,(xx+.5f)/2-.5f,(yy+.5f)/2-.5f);
            if(fault==2 && c==2)v*=.5f;
            output[(size_t(y)*576+x)*3+c]=lut[unsigned(std::lround(v*16383.f))]/65535.f;
        }
        if(fault==3)output[(size_t(100)*576+100)*3]=2;
