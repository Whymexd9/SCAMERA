...
public class CameraUIViewImpl implements CameraUIView {
    ...
    private void initModeSwitcher() {
        CameraMode savedMode = CameraMode.valueOf(PreferenceKeys.getCameraModeOrdinal());
        if (savedMode != CameraMode.MOTION && savedMode != CameraMode.NIGHT) {
            savedMode = CameraMode.MOTION;
            PreferenceKeys.setCameraModeOrdinal(savedMode.ordinal());
        }
        this.mModePicker.setValues(Arrays.stream(CameraMode.userModeNameIds()).map(cameraFragment.activity::getString).toArray(String[]::new));
        this.mModePicker.setOverScrollMode(View.OVER_SCROLL_NEVER);
        this.mModePicker.setOnItemSelectedListener(index -> switchToMode(CameraMode.userModeAt(index)));
        this.mModePicker.setSelectedItem(CameraMode.userModeIndex(savedMode));
    }
