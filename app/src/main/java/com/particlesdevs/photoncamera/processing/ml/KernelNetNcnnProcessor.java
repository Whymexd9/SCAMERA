...
public final class KernelNetNcnnProcessor {
    ...
    public KernelNetNcnnProcessor(Context context) {
        ...
        long h = 0;
        requestedBackend = Math.max(0, Math.min(3,
                PreferenceKeys.getProcessingBackendValue()));
        try {
            Log.i(TAG, "Creating KernelNet backend=" + backendName(requestedBackend));
            h = nativeCreate(am, MODEL_PARAM, requestedBackend);
        } catch (Throwable t) {
    ...
    public Result runInference(FloatBuffer gray, int width, int height, float sigma) {
        ...
        Log.i(TAG, backendName(requestedBackend) + " inference " + width + "x" + height + " -> "
