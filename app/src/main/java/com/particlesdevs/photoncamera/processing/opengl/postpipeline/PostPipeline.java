...
public class PostPipeline extends GLBasePipeline {
    ...
    @Tunable(
        title = "Demosaicing algorithm",
        description = "Selects the Bayer GPU demosaic. Quad Bayer and monochrome use dedicated safe paths.",
        category = "Demosaic",
        entries = {"Compatibility (fast)", "Gradient + chroma (GPU)", "Compute edge-aware (GPU)", "AMaZE RT (maximum quality)"},
        entryValues = {"compat", "gradient", "compute", "amaze"}
    )
    String demosaicingMethod = "amaze";
    ...
    String tonePipeline = "fusion";

    private void computeNoise(Parameters parameters) {
        NoiseModeler modeler = parameters.noiseModeler;
        if (modeler == null || modeler.computeModel == null
                || modeler.computeModel.length < 3
                || modeler.computeModel[0] == null
                || modeler.computeModel[1] == null
                || modeler.computeModel[2] == null) {
            Log.w("PostPipeline", "Missing noise model; using conservative fallback");
            noiseS = 1.0e-4f;
            noiseO = 1.0e-6f;
            return;
        }
    ...
    private void BuildDefaultPipeline() {
        ...
        } else if ("opendrt".equals(tonePipeline)) {
            add(new LinearExposure());
            add(new OpenDRT());
