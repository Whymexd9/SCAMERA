...
public final class FlowNetNcnnProcessor {
    ...
    public static FlowNetNcnnProcessor start(Context context) {
        int requested = Math.max(0, Math.min(3, PreferenceKeys.getProcessingBackendValue()));
        FlowNetNcnnProcessor inst = sInstance;
        if (inst != null && inst.requestedBackend != requested) {
            inst.close();
            inst = null;
        }
        if (inst == null) {
            synchronized (sLock) {
                inst = sInstance;
                if (inst == null) {
                    inst = new FlowNetNcnnProcessor(context, requested);
                    sInstance = inst;
                }
            }
        }
    ...
    private FlowNetNcnnProcessor(Context context, int requestedBackend) {
        appContext = context.getApplicationContext();
        this.requestedBackend = requestedBackend;
        Log.i(TAG, "Creating FlowNet backend=" + backendName(requestedBackend));
    ...
    private static String backendName(int backend) {
        switch (backend) {
            case 1: return "CPU/FP32";
            case 2: return "GPU/Vulkan-FP16";
            case 3: return "NPU request (runtime fallback reported by native layer)";
            default: return "AUTO";
        }
    }
    ...
    private void backgroundInit() {
        ...
        try {
            nativeHandle = nativeCreate(am, MODEL_PARAM, requestedBackend);
        } catch (Throwable t) {
    ...
    private FlowResult runInferenceLocked(FloatBuffer baseRgba, FloatBuffer alterRgba,
        ...
        Log.i(TAG, backendName(requestedBackend) + " inference " + width + "x" + height + " in "
