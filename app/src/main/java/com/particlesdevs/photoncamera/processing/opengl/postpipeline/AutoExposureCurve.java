    ...
    float kneeRef;
        ...
        public void Run() {
            ...
            if (highlightCompression) {
                ...
                float adaptiveKnee = Math2.mix(kneeMax, kneeLo,
                        Math.min(clippedFrac / Math.max(kneeRef, 1.0e-4f), 1.0f));
                float userStrength = Math2.clamp(
                        basePipeline.mParameters.highlightSuppressionStrength, 0.0f, 2.0f);
                if (userStrength <= 1.0f) {
                    knee = Math2.mix(1.0f, adaptiveKnee, userStrength);
                } else {
                    float maximumRecoveryKnee = Math.max(0.12f, adaptiveKnee * 0.35f);
                    knee = Math2.mix(adaptiveKnee, maximumRecoveryKnee,
                            userStrength - 1.0f);
                }
                Log.d(Name, "Highlight shoulder: clipped:" + clippedFrac
                        + " adaptive knee:" + adaptiveKnee
                        + " user strength:" + userStrength + " effective knee:" + knee);
            }
