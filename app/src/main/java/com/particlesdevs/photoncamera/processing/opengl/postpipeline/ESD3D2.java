...
public class ESD3D2 extends Node {
    ...
    @Override
    public void Run() {
        int rtLuma = PreferenceKeys.getRtLumaDenoise();
        int rtChroma = PreferenceKeys.getRtChromaDenoise();
        int rtDetail = PreferenceKeys.getRtDetailRecovery();
        int rtMoire = PreferenceKeys.getRtMoireDenoise();
        Log.i(Name, "RawTherapee-style NR: luma=" + rtLuma
                + " chroma=" + rtChroma + " detail=" + rtDetail
                + " moire=" + rtMoire);
        if (!enable || (rtLuma == 0 && rtChroma == 0 && rtMoire == 0)) {
            WorkingTexture = previousNode.WorkingTexture;
            return;
        }
        float detailKeep = Math2.clamp(rtDetail / 100.0f, 0.0f, 1.0f);
        luma = (rtLuma / 100.0f) * 2.0f * (1.0f - detailKeep * 0.75f);
        useColorDenoising = rtChroma > 0;
