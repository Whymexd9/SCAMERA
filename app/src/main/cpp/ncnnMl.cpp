...
Java_com_particlesdevs_photoncamera_processing_ml_FlowNetNcnnProcessor_nativeCreate(
    JNIEnv* env, jclass, jobject assetManager, jstring paramPath, jint requestedBackend) {
    int64_t t0 = nowMs();
...
#if NCNN_VULKAN
    ctx->net.opt.use_vulkan_compute = requestedBackend != 1;
#else
    ctx->net.opt.use_vulkan_compute = false;
#endif
    if (requestedBackend == 3) {
        // ncnn has CPU/Vulkan backends but no Android NPU backend. Keep the
        // fallback explicit in logcat instead of pretending NNAPI used an NPU.
        LOGI("flownet: NPU requested, no compatible NPU runtime; fallback=%s",
    ...
    if (requestedBackend == 1) {
        // Make CPU an unambiguous pure-CPU reference path. Keeping fp16 flags
        // enabled lets ncnn select packing variants that obscure comparisons
        // and can look similar to the Vulkan path on ARMv8.2 devices.
        ctx->net.opt.use_fp16_packed = false;
        ctx->net.opt.use_fp16_storage = false;
        ctx->net.opt.use_fp16_arithmetic = false;
        LOGI("flownet backend: cpu/fp32 (Vulkan disabled)");
    }
...
Java_com_particlesdevs_photoncamera_processing_ml_KernelNetNcnnProcessor_nativeCreate(
...
#if NCNN_VULKAN
    ctx->net.opt.use_vulkan_compute = requestedBackend == 2 || requestedBackend == 3
            || (requestedBackend == 0 && getenv("KN_GPU") && getenv("KN_GPU")[0] == '1');
#else
    ctx->net.opt.use_vulkan_compute = false;
#endif
    if (requestedBackend == 3) {
        LOGI("kernelnet: NPU requested, no compatible NPU runtime; fallback=%s",
             ctx->net.opt.use_vulkan_compute ? "vulkan" : "cpu");
    }
    ctx->net.opt.use_fp16_packed = true;
    ctx->net.opt.use_fp16_storage = true;
    ...
    if (requestedBackend == 1) {
        ...
        ctx->net.opt.use_fp16_storage = false;
        ctx->net.opt.use_fp16_arithmetic = false;
        LOGI("kernelnet backend requested: cpu/fp32 (Vulkan disabled)");
    }
    pinOpenMPThreads(ctx->net.opt.num_threads);
