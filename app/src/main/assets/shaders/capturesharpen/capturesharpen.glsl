...
void main() {
    ...
    // RawTherapee Amount is a final blend, not a gain hidden inside every
    // iteration. Values above 1 deliberately allow a controlled overshoot.
    float newLuminance = max(oldLuminance + (estimate - oldLuminance)
            * blend * clamp(amount, 0.0, 2.0), 0.0);
