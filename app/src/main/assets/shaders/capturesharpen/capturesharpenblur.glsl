...
float ratioAt(ivec2 point, ivec2 size) {
    ...
    float blurred = max(texelFetch(BlurredEstimate, point, 0).r, epsilon);
    float ratio = original / blurred;
    // Residual-dependent RL damping: corrections smaller than the damping
    // floor are pulled towards one, preventing noise and ringing from growing
    // on every iteration while real edges still converge normally.
    float residual = abs(original - blurred);
    float floorValue = damping * 0.025;
    float weight = (residual * residual) /
            (residual * residual + floorValue * floorValue + epsilon * epsilon);
    return mix(1.0, ratio, weight);
}
