...
void main() {
    ...
    // sigma=0 is the documented bypass: equal exposure weights produce no
    // local exposure selection. Contrast influence is independently tunable;
    // Wronski recommends exposure-only weights for a subtle photographic look.
    vec3 wexp = WEXPOSIGMA <= 0.0001
            ? vec3(1.0)
            : exp(-pow(result.rgb - TARGET, vec3(2.0))
                    / (2.0*WEXPOSIGMA*WEXPOSIGMA));
    vec3 w = wexp * pow(max(lap, vec3(LAPMIN)), vec3(CONTRASTWEIGHT));
