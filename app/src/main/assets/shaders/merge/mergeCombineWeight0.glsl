...
// 0 = short frames are removed upstream, 1 = balanced HDR, 2 = maximum
// retention of the short-frame reference in bright regions.
...
// Native-grid RAW multi-frame super-resolution.  The aligned alter frame is
...
uniform int mergeAlgorithm;
uniform float hdrPlusDenoise;
...
vec4 denoiseConfidence(vec4 confidence, float amount) {
    float a = clamp(amount, 0.0, 2.0);
    vec4 adjusted = pow(clamp(confidence, 0.0, 1.0),
            vec4(mix(1.60, 0.55, min(a, 1.0))));
    adjusted *= min(a, 1.0);
    if (a > 1.0) adjusted = mix(adjusted, sqrt(adjusted), a - 1.0);
    return adjusted;
}
...
void main() {
    ...
    if (mergeAlgorithm == 1) {
        ...
        // moving regions while retaining statistically repeatable detail.
        vec4 lowResidual = localDiffSigned / Z;
        vec4 highResidual = (diff - base) - lowResidual;
        vec4 n2 = max(N * N, vec4(EPS));
        vec4 lowConfidence = n2 / (lowResidual * lowResidual + n2);
        vec4 highConfidence = n2 / (highResidual * highResidual + n2);
        vec4 lowFiltered = denoiseConfidence(lowConfidence, hdrPlusLowDenoise);
        ...
        // CFA-quad chroma consistency gate.  It compares each colour sample's
        // residual with the neutral residual of the complete Bayer quad. At
        // zero the gate is bypassed; larger values increasingly reject frames
        // whose aligned colour residual is inconsistent (colour speckle).
        vec4 residual = diff - base;
    ...
    if (rawMfsr == 1) {
        ...
        // aligned frame contain the same edge polarity and comparable
        // amplitude.  Random RAW noise, zipper colour and alignment errors do
        // not satisfy that consensus and therefore cannot be sharpened.
        vec4 baseMean = vec4(0.0);
        vec4 diffMean = vec4(0.0);
        ivec2 srSize = imageSize(diffTexture);
        for (int my = -1; my <= 1; ++my) {
            for (int mx = -1; mx <= 1; ++mx) {
                ivec2 basePoint = clamp(xy + ivec2(mx, my),
                        ivec2(0), srSize - ivec2(1));
                ivec2 diffPoint = clamp(xy + flow + ivec2(mx, my),
                        ivec2(0), srSize - ivec2(1));
                baseMean += imageLoad(inTexture, basePoint);
                diffMean += imageLoad(diffTexture, diffPoint);
            }
        }
        baseMean /= 9.0;
        diffMean /= 9.0;
        vec4 baseDetail = base - baseMean;
        vec4 diffDetail = diff - diffMean;
        vec4 consensus = min(abs(baseDetail), abs(diffDetail));
        vec4 samePolarity = step(vec4(0.0), baseDetail * diffDetail);
        vec4 disagreement = abs(baseDetail - diffDetail)
                / max(sigmaR, vec4(1e-5));
        vec4 agreement = vec4(1.0) - smoothstep(vec4(0.75), vec4(2.5), disagreement);
        // At least ~2.5 sigma is required: this deliberately favours a clean
        // image over an apparently sharper but noisier one.
        vec4 edgeGate = smoothstep(2.5 * sigmaR, 5.0 * sigmaR, consensus);
        vec4 detail = sign(baseDetail) * consensus;
        float srGain = clamp(rawMfsrStrength, 0.0, 0.30)
                * min(weight * 2.0, 1.0);
        fused = clamp(fused + detail * samePolarity * agreement
                        * edgeGate * comb * srGain,
                vec4(0.0), vec4(1.0));
    }
    if (exposure < 0.95 && hs > 0.0) {
        // The alter is the longer exposure scaled into the short-frame linear
        // domain. At sensor white its value is exactly `exposure`. Select the
        // aligned short reference independently for every clipped Bayer/RGB
        // channel; a clipped red channel must not discard valid green/blue.
        // 0..100 moves from disabled to strict white-level replacement.
        // 100..200 progressively protects near-clipped channels as well.
        float clipLevel = exposure * (hs <= 1.0
                ? 0.9995
                : mix(0.9995, 0.90, hs - 1.0));
        vec4 clipped = step(vec4(clipLevel), diff);
        vec4 recovered = max(base, fused); // linear "lighten", channel-wise
        fused = mix(fused, recovered, clipped * min(hs, 1.0));
    }
    imageStore(outTexture, xy, fused);
