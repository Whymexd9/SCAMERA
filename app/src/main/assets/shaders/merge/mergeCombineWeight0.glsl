...
// GCam/Cyclops forward-backward flow rejection mask generated by FlowNetAlignment.
uniform sampler2D occlusionTexture;
uniform int cyclopsOcclusion;
...
void main() {
    ...
    // Cyclops occlusion is a spatially cleaned rejection mask: 0 means the
    // forward/backward flow is reversible and 1 means the donor must fall back
    // to the reference. GCam subtracts this mask in its final blend; in this
    // confidence-form merge the equivalent safe operation is to cap donor
    // confidence by (1 - occlusion).
    if (cyclopsOcclusion == 1) {
        vec2 occUv = (vec2(xy) + vec2(0.5)) / vec2(imageSize(inTexture));
        float occ = clamp(texture(occlusionTexture, occUv).r, 0.0, 1.0);
        comb = min(comb, vec4(1.0 - occ));
    }
