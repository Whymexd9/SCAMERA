...
#define ACES_ENABLED 0
#define ACES_OUTPUT_P3 0
#define ACES_EXPOSURE 0.0
#define ACES_PEAK 100.0
#define ACES_GAMUT 1.0
#define ACES_HIGHLIGHT_DESAT 1.0
#define ACES_SURROUND 1.0
#define ACES_TONE_CURVE 0
#define ACES_GAMMA_CURVE 0
#define ACES_TONE_MIX 1.0
#define ACES_MID_GRAY 0.18
#define ACES_TONE_CONTRAST 1.0
#define ACES_TOE 0.5
#define ACES_SHOULDER 0.5
#define ACES_HUE_PROTECTION 1.0
#define ACES_CUSTOM_GAMMA 2.2
#define DARKTABLE_ENABLED 0
#define DT_EXPOSURE 0.0
#define DT_FILMIC_CONTRAST 1.0
#define DT_SHADOWS 0.0
#define DT_HIGHLIGHTS 0.0
#define DT_LOCAL_CONTRAST 0.0
#define DT_COLORFULNESS 0.0
#define DT_HIGHLIGHT_RECON 0.0
#define DT_DIFFUSE_SHARPEN 0.0
#define DT_PROFILED_DENOISE 0.0
#define DT_TONE_SHADOWS 0.0
#define DT_TONE_MIDTONES 0.0
#define DT_TONE_HIGHLIGHTS 0.0
#define DT_BALANCE_SHADOWS 0.0
#define DT_BALANCE_MIDTONES 0.0
#define DT_BALANCE_HIGHLIGHTS 0.0
#define DT_COLOR_RECON 0.0
#define DT_CALIB_TEMP 0.0
#define DT_CALIB_TINT 0.0
#define DT_COLOR_RED 0.0
#define DT_COLOR_GREEN 0.0
#define DT_COLOR_BLUE 0.0
#define DT_WIDE_CA 0.0
#define DT_VIGNETTE 0.0
#define DT_HAZE 0.0
...
float acesDither(ivec2 p) {
    ...
    // visible; existing sensor grain remains dominant.
    vec2 q=vec2(p);
    float a=fract(52.9829189*fract(dot(q,vec2(0.06711056,0.00583715))));
    float b=fract(52.9829189*fract(dot(q+vec2(17.0,59.0),vec2(0.06711056,0.00583715))));
    return (a-b)*0.5;
}
...
vec3 applyColorSpace(vec3 pRGB,float tonemapGain, float gainsVal){
    ...
    vec3 ap1 = SRGB_TO_AP1 * pRGB;
    ap1 *= exp2(ACES_EXPOSURE) * mix(tonemapGain, 1.0, LTMMIX);
    ...
    #endif
    vec3 encoded=acesEncode(displayLinear);
    float shadowDither=(1.0-smoothstep(0.04,0.30,mappedY))/255.0;
    encoded+=vec3(acesDither(ivec2(gl_FragCoord.xy))*shadowDither);
    return clamp(encoded,0.0,1.0);
    #else
    // --- Legacy SDR display path ---
...
void main() {
    ...
    // Profiled denoise and Diffuse or Sharpen operate in the scene-linear
    // buffer before the display transform. Edge confidence prevents colour
    // bleeding; NOISES/NOISEO supply the capture-specific ISO noise profile.
    ivec2 imax = textureSize(InputBuffer, 0) - ivec2(1);
    vec3 cross = (
        texelFetch(InputBuffer, clamp(xy+ivec2( 1, 0),ivec2(0),imax),0).rgb +
        texelFetch(InputBuffer, clamp(xy+ivec2(-1, 0),ivec2(0),imax),0).rgb +
        texelFetch(InputBuffer, clamp(xy+ivec2( 0, 1),ivec2(0),imax),0).rgb +
        texelFetch(InputBuffer, clamp(xy+ivec2( 0,-1),ivec2(0),imax),0).rgb) * 0.25;
    float delta = abs(luminocity(sRGB)-luminocity(cross));
    float noiseSigma = sqrt(max(NOISEO + NOISES*luminocity(sRGB), 1e-7));
    float edgeConfidence = exp(-delta/max(4.0*noiseSigma, 1e-4));
    float dn = clamp(DT_PROFILED_DENOISE*edgeConfidence,0.0,1.0);
    sRGB = mix(sRGB,cross,dn*0.75);
    float ds = DT_DIFFUSE_SHARPEN;
    sRGB = ds >= 0.0
        ? max(sRGB + (sRGB-cross)*ds*edgeConfidence,vec3(0.0))
        : mix(sRGB,cross,clamp(-ds,0.0,1.0)*0.8);
    sRGB = max(sRGB + (sRGB-cross)*DT_TEXTURE*0.65*edgeConfidence,vec3(0.0));

    // Radial lateral-CA correction for exceptionally wide coloured fringes.
    vec2 radial = vec2(xy)/vec2(max(imax,ivec2(1))) - 0.5;
